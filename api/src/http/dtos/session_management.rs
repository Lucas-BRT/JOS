...
//
// Session Checkin DTOs
//

#[derive(Deserialize, Serialize, ToSchema, Validate)]
pub struct CreateSessionCheckinRequest {
    // The id of the session intent this checkin refers to
    pub session_intent_id: Uuid,
    // Whether the player attended
    pub attendance: bool,
    // Optional notes (max length may be enforced by validator in real impl)

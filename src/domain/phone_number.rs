use crate::{domain::type_wraper::TypeWrapped, error::PhoneNumberValidationError};
use serde::{Deserialize, Serialize};

#[derive(Debug, PartialEq, Eq, Clone, Serialize, Deserialize)]
pub struct PhoneNumber(String);

impl TypeWrapped for PhoneNumber {
    type Raw = String;
    type Error = PhoneNumberValidationError;

    fn parse(raw: Self::Raw) -> Result<Self, Self::Error> {
        let trimmed_raw = raw.trim();

        if !trimmed_raw.starts_with("+55") {
            return Err(PhoneNumberValidationError::Invalid);
        }
        ...
        if normalized_number.len() < 13 {
            return Err(PhoneNumberValidationError::TooShort);
        }

        if normalized_number.len() > 14 {
            return Err(PhoneNumberValidationError::TooLong);
        }
...
mod tests {
    ...
    fn test_valid_brazilian_phone_numbers() {
        ...
        for input in inputs {
            let result = PhoneNumber::parse(input.to_string());
    ...
    fn test_invalid_prefix() {
        ...
        let result = PhoneNumber::parse(input.to_string());
        assert_eq!(result, Err(PhoneNumberValidationError::Invalid));
    }
    ...
    fn test_invalid_length_too_short() {
        let input = "+55 11 123";
        let result = PhoneNumber::parse(input.to_string());
        assert_eq!(result, Err(PhoneNumberValidationError::TooShort));
    }
    ...
    fn test_invalid_length_too_long() {
        ...
        let result = PhoneNumber::parse(input.to_string());
        assert_eq!(result, Err(PhoneNumberValidationError::TooLong));
    }

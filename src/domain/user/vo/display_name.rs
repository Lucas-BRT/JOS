use super::{MAX_DISPLAY_NAME_LENGTH, MIN_DISPLAY_NAME_LENGTH};
use crate::domain::user::error::{DisplayNameDomainError, UserDomainError};
use crate::domain::utils::type_wraper::TypeWrapped;
use serde::{Deserialize, Serialize};
use unicode_segmentation::UnicodeSegmentation;

static DISPLAY_NAME_REGEX: &str = r"^[a-zA-Z0-9_ ]{3,20}$";
...
mod tests {
    ...
    fn should_allow_valid_display_names() {
        ...
        valid_display_names.iter().for_each(|display_name| {
            assert!(
                DisplayNameVo::parse(display_name.to_string()).is_ok(),
    ...
    fn should_fail_if_display_name_is_too_short() {
        ...
        invalid_display_names.iter().for_each(|display_name| {
            assert!(
                DisplayNameVo::parse(display_name.to_string()).is_err(),
                "Testing display name: {}",
                display_name
            );
        });
    ...
    fn should_fail_if_display_name_is_too_long() {
        ...
        assert!(
            DisplayNameVo::parse(too_long).is_err(),
    ...
    fn should_fail_if_display_name_is_empty_or_whitespace() {
        ...
        empty_cases.iter().for_each(|case| {
            assert!(
                DisplayNameVo::parse(case.to_string()).is_err(),
    ...
    fn should_accept_unicode_composed_characters() {
        ...
        assert!(
            DisplayNameVo::parse(composed.to_string()).is_ok(),
    ...
    fn raw_should_return_original_trimmed_string() {
        let name = "   Alice 🚀   ";
        let parsed = DisplayNameVo::parse(name.to_string()).unwrap();
    ...
    fn should_handle_tabs_and_newlines_correctly() {
        ...
        let result = DisplayNameVo::parse(weird_whitespace.to_string());
        assert!(result.is_ok());
    ...
    fn should_fail_if_only_invisible_characters() {
        ...
        assert!(
            DisplayNameVo::parse(invisible.to_string()).is_err(),
    ...
    fn should_accept_display_names_with_exactly_min_and_max_length() {
        ...
        assert!(
            DisplayNameVo::parse(min.to_string()).is_ok(),
            "Min length check failed"
        );
        assert!(
            DisplayNameVo::parse(max.to_string()).is_ok(),
    ...
    fn should_handle_emoji_as_single_grapheme_properly() {
        ...
        assert!(
            DisplayNameVo::parse(five_emojis.into()).is_ok(),
            "Emoji min length check failed"
        );
        assert!(
            DisplayNameVo::parse(thirty_emojis.clone()).is_ok(),
            "Emoji max length check failed"
        );

        let too_many_emojis = "🚀".repeat(31);
        assert!(
            DisplayNameVo::parse(too_many_emojis).is_err(),
